...
import { MaintenanceData } from "Common/data/dashboard"; // Assuming this is where your data is stored
...
const MaintenanceWarnings = () => {
    ...
    const filterSearchData = (e: any) => {
        ...
        const keysToSearch = ['equipmentName', 'lastMaintained', 'nextMaintenanceDue', 'predictedIssue', 'status'];
        filterDataBySearch(MaintenanceData, search, keysToSearch, setData);
    };
    ...
    const columns = useMemo(() => [
        ...
        },
        {
            ...
            cell: (cell: any) => (
                <>
                    <Link to="/farm-maintenance-overview">{cell.row.original.equipmentName}</Link>
                </>
            ),
        },
        {
            header: "Last Maintained",
            accessorKey: "lastMaintained",
            enableColumnFilter: false,
            enableSorting: true,
        },
        {
        ...
        },
        {
            ...
            cell: (cell: any) => (
                <>
                    {cell.row.original.status === "Critical" ? (
                    ...
                    ) : cell.row.original.status === "Warning" ? (
            ...
            cell: (cell: any) => (
                <>
                    <Dropdown className="relative">
                        <Dropdown.Trigger id="maintenanceAction" data-bs-toggle="dropdown" className="flex items-center justify-center size-[30px] dropdown-toggle p-0 text-slate-500 btn bg-slate-100 hover:text-white hover:bg-slate-600 focus:text-white focus:bg-slate-600 focus:ring focus:ring-slate-100 active:text-white active:bg-slate-600 active:ring active:ring-slate-100 dark:bg-slate-500/20 dark:text-slate-400 dark:hover:bg-slate-500 dark:hover:text-white dark:focus:bg-slate-500 dark:focus:text-white dark:active:bg-slate-500 dark:active:text-white dark:ring-slate-400/20">
                            <MoreHorizontal className="size-3"></MoreHorizontal>
                        </Dropdown.Trigger>
                        <Dropdown.Content placement={cell.row.index ? "top-end" : "right-end"} className="absolute z-50 py-2 mt-1 ltr:text-left rtl:text-right list-none bg-white rounded-md shadow-md dropdown-menu min-w-[10rem] dark:bg-zink-600" aria-labelledby="maintenanceAction">
                            <li>
                                <Link className="block px-4 py-1.5 text-base transition-all duration-200 ease-linear text-slate-600 dropdown-item hover:bg-slate-100 hover:text-slate-500 focus:bg-slate-100 focus:text-slate-500 dark:text-zink-100 dark:hover:bg-zink-500 dark:hover:text-zink-200 dark:focus:bg-zink-500 dark:focus:text-zink-200" to="/farm-maintenance-overview"><Eye className="inline-block size-3 ltr:mr-1 rtl:ml-1"/> <span className="align-middle">Overview</span></Link>
                            </li>
                            <li>
                ...
                <div className="card-body">
                    <div className="grid items-center grid-cols-1 gap-3 mb-5 2xl:grid-cols-12">
                        <div className="2xl:col-span-3">
                            <h6 className="text-15">Maintenance Warnings</h6>
                        </div>
